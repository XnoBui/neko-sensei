...
NGINX_BACKUP_DIR="/root/nginx-backups"
...
if grep -q "/neko-sensei/" "${NGINX_SITE}"; then
  ...
  # Write backup OUTSIDE sites-enabled/ — nginx loads every file in that dir.
  mkdir -p "${NGINX_BACKUP_DIR}"
  cp "${NGINX_SITE}" "${NGINX_BACKUP_DIR}/default-server.bak.$(date +%s)"
